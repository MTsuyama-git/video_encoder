...
JSAMPARRAY img = nullptr;
uint32_t frame_size = 0;
FILE* tmp = nullptr;
uint8_t* tmpArea = nullptr;
int main(int argc, const char** argv)
{
    double fps = 0.0;
    float mspf = 1.0;
    const uint16_t fmt_id = 0xC0DE;
    ...
    static uint16_t dst_width = 480, dst_height = 320;
    ...
    AVStream* st = NULL;
    tmpArea = (uint8_t*)malloc(sizeof(uint8_t*)*dst_width*dst_height*3);
    tmp = fmemopen(tmpArea, sizeof(uint8_t*)*dst_width*dst_height*3, "wb");
    fseek(tmp, 0, SEEK_SET);

    img = (JSAMPARRAY) malloc(sizeof(JSAMPROW) * dst_height);;
    ...
    for(int i = 0; i < dst_height; ++i) {
        img[i] = (JSAMPROW)malloc(sizeof(JSAMPLE) * 3 * dst_width);
    }
    if (ret < 0) {
   ...
   /* For some codecs, such as msmpeg4 and mpeg4, width and height
    ...
    pSwsCtx = sws_getContext(
        ...
        dst_width, dst_height, AV_PIX_FMT_RGBA, // dest Width Height pixFmt
        SWS_BICUBIC, 
        NULL,    // srcFilter
        NULL,    // destFiletr
        NULL     // param
...
void dumpFrame(uint8_t** video_dst_data, int* video_dst_linesize, int width, int height, FILE* f) {
    ...
    for(int y = 0; y < height; ++y) {
        uint8_t* ptr = video_dst_data[0]+y*video_dst_linesize[0];
        for(int x = 0; x < width; ++x) {
            img[y][3 * x + 0] = ptr[4*x+0];
            img[y][3 * x + 1] = ptr[4*x+1];
            img[y][3 * x + 2] = ptr[4*x+2];
        }
    }
    jpeg_write_scanlines(&cinfo, img, height);
    jpeg_finish_compress(&cinfo);
    int frame_size = ftell(tmp);
    // dump size
    fwrite(&frame_size, sizeof(int), 1, f);
    // dump frame jpeg
    fwrite(tmpArea, sizeof(uint8_t), frame_size, f);
